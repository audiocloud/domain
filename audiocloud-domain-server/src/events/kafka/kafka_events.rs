...
pub async fn init(topic: String, brokers: String, username: String, password: String) -> anyhow::Result<()> {
    KAFKA_DOMAIN_EVENTS_SINK.set(Supervisor::start(move || KafkaDomainEventsSink { topic,
                                                                                   brokers,
                                                                                   username,
                                                                                   password,
                                                                                   producer: None }))
...
impl Supervised for KafkaDomainEventsSink {
    fn restarting(&mut self, ctx: &mut <Self as Actor>::Context) {
        ...
        self.producer = Some(BaseProducer::from_config_and_context(&config, DefaultProducerContext)?);
    }
}
...
impl Handler<NotifyDomainEvent> for KafkaDomainEventsSink {
    ...
    fn handle(&mut self, msg: NotifyDomainEvent, ctx: &mut Self::Context) -> Self::Result {
        match self.producer.as_mut() {
            Some(producer) => match Json.serialize(&msg) {
                Ok(encoded) => {
                    if let Err(error) = producer.send(BaseRecord::to(&self.topic).payload(encoded.as_bytes())) {
