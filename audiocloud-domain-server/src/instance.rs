...
use audiocloud_api::cloud::domains::{DomainConfig, DomainFixedInstance};
...
use crate::db::instance::{InstancePlay, InstancePower};
use crate::task::messages::NotifyTaskSpec;
use crate::{instance, nats};

pub mod messages;

static INSTANCE_SUPERVISOR: OnceCell<Addr<InstancesSupervisor>> = OnceCell::new();

pub fn init(cfg: &DomainConfig) -> anyhow::Result<()> {
    let supervisor = InstancesSupervisor::new(cfg)?;
...
impl Instance {
    #[instrument(skip_all)]
    pub fn new(id: FixedInstanceId, spec: DomainFixedInstance, model_spec: Model) -> Self {
        debug!(%id, "Creating new instance actor");
        ...
        Self { id:               id.clone(),
               ...
               command_subject:  format!("ac.isnt.{}.{}.{}.cmds", &id.manufacturer, &id.name, &id.instance), }
    }
    ...
    #[instrument(skip_all)]
    fn send_dirty_parameters(&mut self, ctx: &mut Context<Instance>) {
        ...
        if !rv.is_empty() {
            ...
            nats::request_json(&self.command_subject, cmd).map(Self::handle_instance_driver_error)
                                                          .wait(ctx);
        }
    }
        ...
        if let Some(play) = &mut self.play {
            if !play.state.value().satisfies(play.desired.value()) {
                if play.tracker.should_retry() {
                    ...
                    nats::request_json(&self.command_subject, cmd).into_actor(self)
        ...
        if let Some(power) = &mut self.power {
            ...
            if !channel_power_state.satisfies(*power.desired.value()) {
                if power.tracker.should_retry() {
                    // TODO: audiocloud_models::netio::PowerPdu4CParameters
                    let parameters = { todo!("need") };

                    InstancesSupervisor::from_registry().do_send(SetInstanceParameters { instance_id: power.spec
                                                                                                           .instance
                                                                                                           .clone(),
...
impl Handler<NotifyTaskSpec> for Instance {
    type Result = ();

    fn handle(&mut self, msg: NotifyTaskSpec, ctx: &mut Self::Context) -> Self::Result {
        let mut changed = false;
        if self.owner.as_ref().map(|o| o == &msg.task_id).unwrap_or_default() {
...
impl Handler<SetInstanceParameters> for Instance {
    type Result = ();
...
impl Handler<SetInstanceDesiredState> for Instance {
    type Result = ();
...
impl Handler<NotifyInstancePower> for Instance {
    type Result = ();
...
impl Handler<NotifyInstanceDriver> for Instance {
    type Result = ();
...
pub struct InstancesSupervisor {
    instances: HashMap<FixedInstanceId, Addr<Instance>>,
}

impl InstancesSupervisor {
    pub fn new(boot: &DomainConfig) -> anyhow::Result<Self> {
        let mut instances = HashMap::new();
        for (id, instance) in &boot.fixed_instances {
            let actor = Instance::new(id.clone(),
                                      instance.clone(),
                                      boot.models
                                          .get(&id.model_id())
