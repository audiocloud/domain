...
pub fn setup_prometheus(opts: &O11yOpts) -> anyhow::Result<()> {
    let controller =
        controllers::basic(processors::factory(selectors::simple::inexpensive(),
                                               cumulative_temporality_selector()).with_memory(true)).with_resource(opts.generate_tracing_resource()).build();
